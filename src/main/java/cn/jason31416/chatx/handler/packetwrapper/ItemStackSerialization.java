...
public final class ItemStackSerialization {
    ...
    private static ItemStack readModern(PacketWrapper<?> wrapper, boolean lengthPrefixed) {
        ...
        try {
            itemType = wrapper.readMappedEntity(ItemTypes.getRegistry());
    ...
    public static @Nullable HashedStack readHashedStack(PacketWrapper<?> wrapper) {
        if (!wrapper.readBoolean()) return null;
        ItemType item = ItemTypes.STONE;
        try{
            item = wrapper.readMappedEntity(ItemTypes.getRegistry());
        ...
        try {
            ...
            return new HashedStack(item, count, new HashedComponentPatchMap(Map.of(), Set.of()));
        }
    }
}
