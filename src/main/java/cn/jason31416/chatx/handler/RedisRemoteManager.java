...
public class RedisRemoteManager {
    ...
    public RedisRemoteManager(){
        ...
        try {
            ...
            updateTask = ChatX.getProxy().getScheduler().buildTask(ChatX.getInstance(), ()->{
                jedis.hset("servers", Config.getString("server-name"), String.valueOf(System.currentTimeMillis()));
                jedis.set("onlines."+Config.getString("server-name"), String.join(",", ChatX.getProxy().getAllPlayers().stream().map(Player::getUsername).toList()));
                updateOnlinePlayers();
